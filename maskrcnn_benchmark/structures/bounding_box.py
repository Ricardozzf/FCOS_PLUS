...
class BoxList(object):
    ...
    def convert(self, mode):
        ...
        if self.vis:
            ...
            tuple_add = ()
        if mode == "xyxy":
            bbox = torch.cat((xmin, ymin, xmax, ymax)+tuple_add, dim=-1)
            bbox = BoxList(bbox, self.size, mode=mode)
        else:
            TO_REMOVE = 1
            bbox = torch.cat(
                (xmin, ymin, xmax - xmin + TO_REMOVE, ymax - ymin + TO_REMOVE)+tuple_add, dim=-1
            )
            bbox = BoxList(bbox, self.size, mode=mode)
    ...
    def resize(self, size, *args, **kwargs):
        ...
        scaled_box = torch.cat(
            (scaled_xmin, scaled_ymin, scaled_xmax, scaled_ymax)+tuple_add, dim=-1
        )
    ...
    def transpose(self, method):
        ...
        if self.vis:
            ...
            if method == FLIP_LEFT_RIGHT:
                TO_REMOVE = 1
                v_w[indice] =  image_width - v_w[indice] - TO_REMOVE
            elif method == FLIP_TOP_BOTTOM:
                v_h[indice] = image_height - v_h[indice]  
            tuple_add = (v_w, v_h)
        else:
            xmin, ymin, xmax, ymax = self._split_into_xyxy()
            tuple_add = ()
    ...
    def crop(self, box):
        ...
        cropped_box = torch.cat(
            (cropped_xmin, cropped_ymin, cropped_xmax, cropped_ymax)+tuple_add, dim=-1
        )
